...
Логін, що хешує лише коли акаунт існує, відповідає «немає такого акаунта» на
два порядки швидше за «невірний пароль», і однакові тексти помилок цього не
ховають: ендпоїнт стає списком зареєстрованих адрес, читабельним секундоміром.
`BurnVerify` витрачає ту саму роботу на гілці відсутнього акаунта:

```go
var decoy string // будується раз на старті: hasher.Hash(randomBytes)
...
Decoy має походити від того самого хешера з тими самими налаштуваннями, інакше
гілки знову коштують по-різному; `NeedsRehash` тримає довгоживучий decoy у
синхроні. З порожнім decoy хелпер хешує пароль замість тихого повернення - щоб
оракул не відкривався саме на неправильно налаштованому деплої. Він вирівнює
рівно цю різницю й нічого більше: обмеження частоти та однакові відповіді
лишаються за вами.
...
вашою БД. Кожен запис несе `IssuedAt` поруч з `ExpiresAt`, обидва з одного
зчитування годинника - саме це потрібно і списку сесій, і «відкликати все,
видане до зміни пароля».

### Необов'язкові половини

Три речі, яких потребує повний refresh-цикл, - необов'язкові інтерфейси поруч
з обов'язковим контрактом, кожен із пакетною функцією, що працює в обидва боки
й повертає `ErrUnsupported`, коли стор цього не вміє:
...
`RotateWithStatus` каже, чим була спроба:

| Статус | Значення | Відповідь |
|---|---|---|
| `Rotated` | токен був чинний; видано наступника | нова пара |
| `PreviousWithinGrace` | безпосередньо попередній токен, повторно, у вікні | 401; не карати |
| `ReusedStale` | давніший чи відкликаний токен | відкликати родину |
| `RotateUnknown` | без вердикту; вся історія - в помилці | обробити помилку |

Нульове значення - `RotateUnknown`, свідомо: результат подорожує поруч із
помилкою, і нуль, що читався б як успіх, винагороджував би саме того викликача,
який глянув на статус раніше за помилку. `RotateResult.Subject` називає
власника, коли стор ще знає, - щоб відповіді на `ReusedStale` було за що
відкликати.

`PreviousWithinGrace` - не автентифікація. На момент звіту запис попереднього
токена - разом із хешем секрету - вже зник, тож ніхто не перевірив, що поданий
секрет належить цьому id. Це означає «клієнт з обірваним з'єднанням повторює
себе; відповісти 401 і не відкликати родину», ніколи - «пред'явник той, ким
називається».

На сторі без `GraceRotator` `RotateWithStatus` ніколи не вигадує грейс: успіх
стає `Rotated`, а `ErrRefreshUsed` - `ReusedStale`. Стор, що не може довести
різницю, не змушують її стверджувати.

### Еталон у пам'яті

`NewMemoryRefreshStore` реалізує весь контракт, включно з необов'язковими
половинами, і форсить строк дії: прострочений токен не ротується
(`ErrRefreshExpired` - свідомо не сигнал повторного використання, бо лінивий
клієнт не злодій), `Get` трактує прострочений запис як відсутній, мертві записи
вичищаються. Він для тестів і однопроцесних програм; `WithGrace(d)` відкриває
грейс-вікно, інакше воно вимкнене.
